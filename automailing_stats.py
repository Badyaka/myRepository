...
(
		...
		from
			...
			from (
					...
					from mailings_test m inner join mail_id mi on m.psource = mi.psource and m.mail_id = mi.mid
					...
					-- TRANSACTIONS (total, BEFORE and DURING event)-----------------------------------------------------------------
					left join (
							select t.psource, t.uid, t.stdate as tdate, t.currency, t.amt_usd/100 amt_usd, md2.mail_name, md2.ptype,
                    ...
                    (select raw.*, (avg(t.amt_usd/100)*3/4 + min(t.amt_usd/100)*1/4) CB_amt_usd
						...
						left join transactions t on raw.uid = t.uid and raw.currency = t.currency and raw.psource = t.psource
...
def get_mailings(psource):
    #  Connect to the database 'mailing'
    conn = connect(host='localhost', user='root', password=get_pass(), db='mailing', charset='utf8mb4',
                   cursorclass=cursors.DictCursor)
    c = conn.cursor()
    sql = "select distinct event from promo_header where locale = 'totals' and psource = " + str(psource)
    c.execute(sql)
    r = c.fetchall()
    sqldata = []
    for elem in r:
        sqldata.append(elem['event'])
    return sqldata


def get_one_mailing_stats(event_name, time_frame, psource):
    #  Connect to the database 'mailing'
    conn = connect(host='localhost', user='root', password=get_pass(), db='mailing', charset='utf8mb4',
                   cursorclass=cursors.DictCursor)
    c = conn.cursor()
    sql = get_query().replace('@ps', str(psource))
...
with open("C:\DUMP_ALL\mail_stats_psource" + str(psource) + ".csv", 'w') as f:
